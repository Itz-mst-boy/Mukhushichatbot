...
async def addchat(_, message): 
    ...
    if not is_kuki:
        ...
        await message.reply_text(f"✅ | Successfully\nMukhushi Chatbot of this Group is set to @{message.chat.username}\n Requested by [{message.from_user.first_name}](tg://user?id={message.from_user.id})\n© @MetaVoid")
    else:
        await message.reply_text(f"Already Setup Mukhushi Chatbot of this Group Is @{message.chat.username}")
...
async def rmchat(_, message): 
    ...
    is_kuki = kuki.find_one({"chat_id": message.chat.id})
    if not is_kuki:
        await message.reply_text("Already Mukhushi ChatBot Disable")
    else:
        kuki.delete_one({"chat_id": message.chat.id})
        await message.reply_text("✅ | Mukhushi ChatBot is disable!")
    ...
    if message.chat.type != "private":
        buttons = InlineKeyboardMarkup(
            [[InlineKeyboardButton(text="Click here",
                url=f"t.me/mukhushichatbot?start")]])
        await message.reply("Contact me in PM",
                            reply_markup=buttons)
        
    else:
        buttons = [[InlineKeyboardButton("Support", url="https://t.me/worldwide_friend_zone"),
                    InlineKeyboardButton("Channel", url="https://t.me/mukhushi_official"),
                    InlineKeyboardButton("Owner", url="https://t.me/itz_mst_boy")
                    ]]
        Photo = "https://telegra.ph/file/3d2fa675f8f049a314572.jpg"
        await message.reply_photo(Photo, caption=f"Hello [{message.from_user.first_name}](tg://user?id={message.from_user.id}), Machine Learning Chat Bot that can talk about any topic in any language\n /help - Help Commands\n Powered By @mr_sukkun", reply_markup=InlineKeyboardMarkup(buttons))
...
async def help(client, message):
    ...
    if message.chat.type != "private":
        buttons = InlineKeyboardMarkup(
            [[InlineKeyboardButton(text="Click here",
                url=f"t.me/mukhushichatbot?start=help_")]])
        await message.reply("Contact me in PM",
                            reply_markup=buttons)
        
    else:    
        await message.reply_text("/start - Start The Bot\n/chat - Send a message to this bot\n/setupchat - Active Mukhushi Chatbot In Group\n/removechat - Disable Mukhushi Chatbot In Group")
